...
export const AuthPage: React.FC<AuthPageProps> = ({ onClose }) => {
  ...
  return (
    <div className="min-h-screen w-screen flex flex-col md:flex-row bg-slate-50 font-sans antialiased overflow-hidden">
      ...
      <div className="flex-1 hidden md:flex flex-col justify-between p-12 bg-gradient-to-tr from-slate-950 via-slate-900 to-indigo-950 relative border-r border-slate-900">
        ...
        <div className="relative z-10 max-w-lg">
          ...
          <p className="text-slate-400 mt-4 text-xs font-semibold leading-relaxed">
      ...
      <div className="w-full md:w-[500px] flex items-center justify-center p-8 bg-slate-50 border-l border-slate-200/50 relative">
        ...
        <div className="w-full max-w-sm relative z-10 bg-white/90 backdrop-blur-xl rounded-3xl ring-1 ring-gray-200/80 p-8 shadow-xl">
          {onClose && (
            <button 
              onClick={onClose}
              className="flex items-center gap-1.5 text-xs font-bold text-slate-500 hover:text-slate-900 transition-colors mb-6 cursor-pointer border-0 bg-transparent"
            >
          ...
          {(error || validationError) && (
            <div className="bg-rose-500/10 border border-rose-500/20 rounded-2xl p-4 flex gap-3 text-xs text-rose-500 mb-6 animate-slide-up">
              ...
              <div>
                <p className="font-bold text-slate-800">Authorization Error</p>
          ...
          {forgotSuccess && (
            <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-4 flex gap-3 text-xs text-emerald-600 mb-6 animate-slide-up">
              ...
              <div className="w-full">
                <p className="font-bold text-slate-800">Token Generated</p>
                ...
                <p className="mt-2 text-[10px] text-slate-400 font-semibold animate-pulse">Redirecting to Reset screen...</p>
              </div>
            </div>
          )}
          ...
          {resetSuccess && (
            <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-4 flex gap-3 text-xs text-emerald-600 mb-6 animate-slide-up">
              <span className="h-4.5 w-4.5 flex-shrink-0 mt-0.5 font-bold">✓</span>
              <div>
                <p className="font-bold text-slate-800">Password Reset</p>
                <p className="mt-0.5 font-semibold text-emerald-600">{resetSuccess}</p>
                <p className="mt-2 text-[10px] text-slate-400 font-semibold animate-pulse">Redirecting to Sign In...</p>
              </div>
            </div>
          )}
          ...
          {(view === 'login' || view === 'signup') && (
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full h-11 bg-white border border-slate-200 rounded-full pl-10.5 pr-4 text-slate-800 text-xs placeholder-slate-400 focus:outline-none focus:border-gray-400 focus:ring-2 focus:ring-slate-100 transition-all"
                    />
                  </div>
                </div>
              )}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full h-11 bg-white border border-slate-200 rounded-full pl-10.5 pr-11 text-slate-800 text-xs placeholder-slate-400 focus:outline-none focus:border-gray-400 focus:ring-2 focus:ring-slate-100 transition-all"
                  />
                  <button
                    type="button"
                    onClick={() => setShowPassword(!showPassword)}
                    className="absolute inset-y-0 right-3.5 flex items-center text-slate-400 hover:text-slate-600 transition-colors cursor-pointer border-0 bg-transparent"
                  >
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full h-11 bg-gray-900 hover:bg-indigo-650 active:bg-gray-950 disabled:bg-slate-200 disabled:text-slate-400 text-white font-bold text-xs rounded-full shadow-sm hover:shadow-md transition-all mt-4 cursor-pointer flex items-center justify-center border-0"
              >
                {loading ? (
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full h-11 bg-white border border-slate-200 rounded-full pl-10.5 pr-4 text-slate-800 text-xs placeholder-slate-400 font-mono focus:outline-none focus:border-gray-400 focus:ring-2 focus:ring-slate-100 transition-all"
                  />
                </div>
              </div>
