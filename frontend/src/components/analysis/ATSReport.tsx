  ...
  return (
    <div className="bg-white/60 backdrop-blur-md rounded-3xl ring-1 ring-gray-200/80 shadow-sm p-6 space-y-5 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-bold font-display text-slate-800">ATS Layout & Diagnostics</h3>
          <p className="text-xs text-slate-500 mt-1">
            Structural analyzer checking compatibility with legacy parsing engines.
          </p>
        </div>
        <div className="text-right">
          <span className="text-[10px] text-slate-400 font-bold block uppercase tracking-widest">Layout Score</span>
      ...
      {issues.length === 0 ? (
        <div className="bg-emerald-50/40 border border-emerald-200/50 rounded-2xl p-6 text-center">
          ...
          <h4 className="font-bold text-slate-800 text-sm">Perfect Layout Compatibility!</h4>
          <p className="text-xs text-slate-500 mt-1">No layout blocks, table grids, or weird custom shapes were flagged.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {issues.map((issue, idx) => (
            <div 
              key={idx} 
              className="border border-slate-200/60 bg-white/40 rounded-2xl p-4 flex items-start gap-3.5 hover:border-slate-350 transition-colors"
            >
              ...
              <div className="flex-1 space-y-1.5">
                ...
                <div className="bg-slate-50/70 rounded-xl p-2.5 text-[11px] text-slate-500 leading-normal border border-slate-150/80">
